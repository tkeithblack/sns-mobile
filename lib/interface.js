...
Interface.prototype.sendMessage = function(endpointArn, msg, callback) {
  ...
  }   } else if (this.platform === SUPPORTED_PLATFORMS.IOS_VOIP) {
    convertFn = this.convertVoIPToApnsFormat;
  } else {
...
Interface.prototype.convertVoIPToApnsFormat = function (message, callback) {
  // APNS format expected by amazon for messages
  // {
  //   APNS: JSON.stringify({
  //     "aps": {
  ...
  var APNS = this.sandbox ? 'APNS_VOIP_SANDBOX' : 'APNS_VOIP'
    , messageContainer = {};
  ...
  if (typeof message === 'string') {
    ...
    if (message['APNS_VOIP_SANDBOX'] || message['APNS_VOIP']) {
      callback(null, message);
    } else {
      try {
        messageContainer[APNS] = JSON.stringify(message);
    ...
    var e = new Error('Unable to convert message to VoIP APNS format. Message' +
      ' must be String/Object.');
